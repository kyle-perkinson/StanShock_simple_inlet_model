...
class Pseudoshock(RightHandSide):
    ...
    def get_shock_location(
        ...
        _: FluidPhysics = physics
        p = state.pressure
        x = geometry.x
        x_iso_start, x_iso_end = geometry.regions["isolator"]
        xf = x[:-1]
        p2p1a = p[1:] / p[:-1]
        sf_options = np.where((p2p1a > 1.15) & (xf >= x_iso_start) & (xf <= x_iso_end))[
            0
        ]
    ...
    def M_Ar_Derivatives(
        self, x, y, g1, q1, k0, kappa, norm_int, cf0, cf_model, Dh_func
    ):
        Dh = Dh_func(x)
        M2, Aratio, p = y
        q = g1 * M2 * p / 2
    ...
    def get_preshock_properties(
        ...
        if len(self.sf_array) >= 2 and not np.isnan(self.sf_array[-2:]).any():
            u_s = (x[self.sf_array[-1]] - x[self.sf_array[-2]]) / (
                self.time_array[-1] - self.time_array[-2]
            )
            if np.abs(u_s) <= state.velocity[sInd]:
                M1 = (
                    np.abs(state.velocity[sInd] - u_s)
                    / physics.get_sound_speed(state)[sInd]
                )
                self.us.append(u_s)
    ...
    def pseudoshock_solver(
        ...
        pseudo_solve = solve_ivp(
            ...
            t_span=[geometry.x[0], geometry.x[-1]],
            y0=y0,
            method="RK45",
            t_eval=geometry.x,
            events=reattachment,
    ...
    def location_optimizer(
        ...
        _, y0 = self.get_preshock_properties(
            time, state, physics, geometry, self.sf_array[-1]
        )
        if np.sqrt(y0[0]) < 1.3:
            return dp_arr
        ...
        if self.t_ps == 0:
            self.t_ps = time
        p_pseudo, x_pseudo, p2, L_ps = self.pseudoshock_solver(
        ...
        error_array = [1, 0]
        while (
            error_array[-1] < error_array[-2] and len(error_array) < 5
        ):  # resolves until shock location is closest match to existing profile
        ...
        plt.plot(
            (x_pseudo + x[self.sf_array[-1]]),
            p_pseudo / state.pressure[0],
            label=r"p_{pseudo}",
        )
